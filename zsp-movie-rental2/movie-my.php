...
<html>
    ...
    <body>
        ...
        <div class="glowna">
            <h4>Twoje filmy:</h4> 
            <?php
            $movies = 'SELECT movies.name, movies.year, movies.specie_id, movies.user_id, movies.admin_id, movies.lessor_id FROM `movies`JOIN users ON movies.user_id = users.id WHERE users.id = "'.$_SESSION['login'].'";';
                if($result_movies = $db -> query($movies))
                {
                    while($row_movies = $result_movies -> fetch_array())
                    {
                        if($row_movies['login'] = $_SESSION['login'])
                        {
